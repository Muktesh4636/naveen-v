export var pad = (n) => String(n).padStart(2, "0");
...
export function formatClock(now = /* @__PURE__ */ new Date()) {
  // Always Indian Standard Time with seconds (HH:MM:SS).
  try {
    ...
    const ist = new Date(now.getTime() + (5.5 * 60 - now.getTimezoneOffset()) * 60e3);
    return `${pad(ist.getUTCHours())}:${pad(ist.getUTCMinutes())}:${pad(ist.getUTCSeconds())}`;
  }
}
export function formatMins(minutes) {
...
export function formatOfcDate(str) {
